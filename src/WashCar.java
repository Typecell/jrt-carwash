...
public class WashCar {
    ...
    Реализовать метод, который будет принимать нового клиента и тип мойки. В этом методе нужно:

создать тикет;

добавить тикет в очередь (при этом, если клиент — «Вип», он встаёт в начало очереди);

рассчитать стоимость услуги, исходя из типа машины и типа мойки;

если это 10-я по счёту мойка для клиента, применить скидку 10%.

Реализовать метод завершения мойки, который:

увеличит счётчик количества моек у клиента;

если количество моек у клиента превысило 100, изменит его статус на «Вип»
     */
}
